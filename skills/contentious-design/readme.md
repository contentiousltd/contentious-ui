...
Hover is a **background step, not a colour shift**: transparent → `--surface-hover` for menu rows and controls (limestone-600 in CHC); limestone-450 for list rows. `--surface-hover` is a signature token each product declares, and it is neutral — a step off the surface being hovered, never the raised band, which is a lichen band on the studio site and the same sorbet stop as the menu on VTS. `--surface-hover` and `--text-on-hover` are also the names shadcn's `--accent` / `--accent-foreground` pair takes here — that collision is settled in `provenance/Accent decision 2026-07-31.html`: `--accent` in this system means the primary interactive colour and keeps the name. Row actions and settings cogs are hidden at rest and revealed on hover. Destructive rows hover to a fire tint.
...
| `tokens/products.css` | **Product signatures.** The closed 29-token set each product may override, the allocation rules, and the recipe for adding the next product. |
